...
class DiseasePredictionService:
    ...
    def _load_disease_info(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            ...
            'Vitiligo': {
                'description': 'A condition causing loss of skin pigment in patches, resulting in white spots on the skin.',
                'symptoms': ['White patches', 'Loss of pigment', 'Premature graying of hair', 'Changes in eye color'],
                'causes': ['Autoimmune disorder', 'Genetic factors', 'Stress', 'Environmental triggers'],
                'treatment': 'Topical corticosteroids, Phototherapy, Depigmentation therapy, Cosmetic camouflage',
                'urgency': 'Low',
                'prevention': 'Sun protection, Stress management, Regular skin monitoring'
            },
            'Acne Vulgaris': {
                'description': 'A common skin condition characterized by pimples, blackheads, and whiteheads.',
                'symptoms': ['Pimples', 'Blackheads', 'Whiteheads', 'Cysts', 'Nodules', 'Oily skin'],
                'causes': ['Excess oil production', 'Clogged pores', 'Bacteria', 'Hormonal changes'],
                'treatment': 'Topical treatments, Oral medications, Antibiotics, Professional procedures',
                'urgency': 'Low',
                'prevention': 'Regular cleansing, Avoid picking, Use non-comedogenic products'
            },
            'Normal Skin': {
                'description': 'Healthy skin without any apparent dermatological conditions.',
                'symptoms': ['Clear complexion', 'Normal texture', 'No lesions or abnormalities'],
                'causes': ['Good skincare routine', 'Healthy lifestyle', 'Genetic factors'],
                'treatment': 'Maintain current skincare routine',
                'urgency': 'None',
                'prevention': 'Continue healthy habits, Regular skin checks'
            },
            'Actinic Keratosis': {
                'description': 'Rough, scaly patches on the skin caused by years of sun exposure.',
                'symptoms': ['Rough patches', 'Scaly texture', 'Red or pink color', 'Itching'],
                'causes': ['UV radiation exposure', 'Sun damage', 'Age', 'Fair skin'],
                'treatment': 'Cryotherapy, Topical medications, Surgical removal, Photodynamic therapy',
                'urgency': 'Medium',
                'prevention': 'Sun protection, Regular skin checks, Avoid tanning beds'
            },
            'Contact Dermatitis': {
                'description': 'Skin inflammation caused by direct contact with irritants or allergens.',
                'symptoms': ['Redness', 'Itching', 'Burning sensation', 'Swelling', 'Blisters'],
                'causes': ['Allergens', 'Irritants', 'Chemicals', 'Plants', 'Metals', 'Cosmetics'],
                'treatment': 'Avoid triggers, Topical corticosteroids, Antihistamines, Moisturizers',
                'urgency': 'Low',
                'prevention': 'Identify and avoid triggers, Use protective clothing, Patch testing'
            }
        }
    ...
    def load_model(self, model_path: str):
        ...
        try:
            ...
            if os.path.exists(labels_path):
                ...
                self.class_to_index = {name: idx for idx, name in enumerate(self.class_names)}
                self.index_to_class = {idx: name for idx, name in enumerate(self.class_names)}
            elif 'best_model.h5' in model_path:
                # Special handling for best_model.h5 - use the training script's DISEASES
                self.class_names = [
                ...
                self.class_to_index = {name: idx for idx, name in enumerate(self.class_names)}
                self.index_to_class = {idx: name for idx, name in enumerate(self.class_names)}
                logger.info(f"Set class names for best_model.h5: {self.class_names}")
    ...
    def predict(self, image: Union[Image.Image, str, bytes], symptoms: List[str] = None, top_k: int = 3) -> List[Dict]:
        ...
        try:
            ...
            model_inputs = [processed_batch]
            
            # Check if model expects multimodal input (image + tabular)
            if hasattr(self.model, 'input') and isinstance(self.model.input, list) and len(self.model.input) > 1:
                # Multimodal model - prepare symptom features
                tabular_features = self._prepare_symptom_features(symptoms or [])
                
                # If using TTA, repeat tabular features to match batch size
                if self.use_tta and processed_batch.shape[0] > 1:
                    tabular_features = np.repeat(tabular_features, processed_batch.shape[0], axis=0)
                
                model_inputs.append(tabular_features)
    ...
    def _prepare_symptom_features(self, symptoms: List[str]) -> np.ndarray:
        """Convert symptoms list to feature vector for multimodal model"""
        # Symptom severity mapping (same as training script)
        severity_mapping = {
        ...
        # Convert selected symptoms to severity levels
        for symptom in symptoms:
            symptom_lower = symptom.lower()
            if 'itch' in symptom_lower:
                symptom_dict['itching'] = 'Moderate'  # Default severity for selected symptoms
            elif 'pain' in symptom_lower:
                symptom_dict['pain'] = 'Moderate'
            elif 'scal' in symptom_lower or 'flak' in symptom_lower:
                symptom_dict['scaling'] = 'Moderate'
            elif 'red' in symptom_lower or 'erythema' in symptom_lower or 'rash' in symptom_lower:
                symptom_dict['erythema'] = 'Moderate'
        
        # Convert to feature vector
        feature_values = [
            severity_mapping.get(symptom_dict.get('itching', 'None'), 0),
            severity_mapping.get(symptom_dict.get('pain', 'None'), 0),
            severity_mapping.get(symptom_dict.get('scaling', 'None'), 0),
            severity_mapping.get(symptom_dict.get('erythema', 'None'), 0)
        ]
        
        # Return as batch (shape: [1, 4])
