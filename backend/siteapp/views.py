...
def _get_prediction_service():
    ...
    if _PREDICTION_SERVICE is None:
        try:
            ...
            model_paths = [
                os.path.join(str(settings.BASE_DIR), 'best_model.h5'),
...
def send_email_otp(request: HttpRequest) -> JsonResponse:
    ...
    # Prefer authenticated SMTP user as from-address to avoid provider rejections (e.g. Gmail)
    from_email = getattr(settings, 'EMAIL_HOST_USER', '') or getattr(
        settings, 'DEFAULT_FROM_EMAIL', 'no-reply@teim.co.in'
    )
    try:
        send_mail(subject, message, from_email, [email], fail_silently=False)
    except Exception as exc:
        logger.warning("Failed to send email OTP: %s", exc)
        # Do NOT expose the OTP in responses; require a real email delivery.
        request.session['email_otp'] = otp_payload
        request.session.pop('email_verified_email', None)
        request.session.modified = True
        return JsonResponse(
            {'ok': False, 'error': 'Could not send OTP email right now. Please try again later.'},
            status=500,
        )
...
def predict(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        try:
            predictions_data = service.predict(img, symptoms=symptoms, top_k=2)
        ...
        # If we have predictions, analyze top prediction
        top_label = predictions[0]['disease'] if predictions else None

        # Case 2: Clear/Normal skin detection
        if top_label and 'normal' in top_label.lower():
